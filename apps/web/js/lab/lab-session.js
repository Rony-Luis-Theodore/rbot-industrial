 ...
 *  1. Ubicar (1×) fija ψ y tip → CONGELA y oculta el botón (anti vistas CAD).
 *  2. Al manejar: SOLO project(odom) + trazo. Cero LiDAR / cero refine / cero Ubicar.
 *  3. Recargar la página = única forma de volver a Ubicar.
...
const state = {
  ...
  needsUbicar: true,
  /** Tras Ubicar OK: botón oculto hasta recargar la página. */
  ubicarLocked: false,
  trailOdom: [],
  trail: [],
  scan: null,
  localizing: false,
  status: "need_ubicar",
  hint: `Centro del circuito → Ubicar (mejor lectura)`,
  listeners: new Set(),
};
...
function clearAnchor(reasonHint) {
  ...
  state.ubicarLocked = false;
  state.trailOdom = [];
  state.trail = [];
  setHint(
    reasonHint || "Centro del circuito → Pulsa «Ubicar en el mapa»",
    "need_ubicar"
  );
}
...
export async function loadLabMap(mapId) {
  ...
  await loadGeometry();
  clearAnchor(bootHint());
}

function bootHint() {
  // Limpiar restos del experimento «Reiniciar ×3 / 4º recarga» (v50).
  try {
    sessionStorage.removeItem("rbot.lab.restartCount");
    sessionStorage.removeItem("rbot.lab.hardRestart");
  } catch {
    /* ignore */
  }
  try {
    ...
    if (nav && nav.type === "reload") {
      return "Programa reiniciado (recarga) — Pulsa «Ubicar en el mapa» una vez";
    }
  } catch {
    /* ignore */
  }
  try {
    if (sessionStorage.getItem("rbot.lab.softRestart") === "1") {
      sessionStorage.removeItem("rbot.lab.softRestart");
      return "Programa reiniciado — Pulsa «Ubicar en el mapa» una vez";
    }
  } catch {
    /* ignore */
  }
  return "Mapa listo — Centro del circuito → Ubicar (recomendado)";
}

/**
 * Reinicio de programa (no es F5 del navegador): limpia ancla/trazo
 * y deja el gate de Ubicar como en un arranque limpio.
 */
export async function restartLabProgram(reason = "manual") {
  state.scan = null;
  state.localizing = false;
  state.ubicarLocked = false;
  clearAnchor(
    reason === "reload"
      ? "Programa reiniciado — Pulsa «Ubicar en el mapa» una vez"
      : "Programa reiniciado — Pulsa «Ubicar en el mapa» una vez"
  );
  try {
    sessionStorage.setItem("rbot.lab.softRestart", "1");
  } catch {
    /* ignore */
  }
  if (state.mapId) {
    await loadLabMap(state.mapId);
  }
  try {
    window.dispatchEvent(
      new CustomEvent("rbot:lab-restart", { detail: { reason } })
    );
  } catch {
    /* ignore */
  }
...
export async function relocalizeLab({ reason = "", auto = false, passes } = {}) {
  ...
  // Ya anclado y bloqueado: no re-Ubicar (evita vistas CAD).
  if (
    UBICAR_ONCE_PER_SESSION &&
    state.ubicarLocked &&
    isLabAnchored()
  ) {
    setHint("Ya anclado — Si falló, recarga la página y Ubica de nuevo", "ok");
    return false;
  }
  ...
  const nPasses = Math.max(1, Number(passes) || 1);
  ...
  try {
    ...
    state.align.mapTh = best.mapTh;
    state.alignFrozen = true;
    state.needsUbicar = false;
    state.ubicarLocked = Boolean(UBICAR_ONCE_PER_SESSION);
    ...
    const lockNote = state.ubicarLocked ? " · ancla fija" : "";
    setHint(
      `Anclado (${t.x.toFixed(2)}, ${t.y.toFixed(2)}) · ψ=${deg}°${lockNote} · Si el LiDAR no cuadra → Recarga la página`,
      "ok"
    );
    return true;
  } catch (err) {
